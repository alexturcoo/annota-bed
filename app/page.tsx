...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!gtfBgz) return setError("Please upload a bgzip-compressed GTF (.gtf.bgz).");
    setError(null);
    setLoading(true);
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border p-5 space-y-5 bg-white/50">
        <div className="grid gap-5 md:grid-cols-3">
          <div className="md:col-span-3">
            <label className="block text-sm font-medium mb-1">Upload BED file</label>
            <input type="file" accept=".bed,.tsv,.csv,.txt"
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Upload GTF (bgzip, e.g. *.gtf.bgz)</label>
            <input type="file" accept=".bgz,.gz"
              onChange={(e) => setGtfBgz(e.target.files?.[0] || null)}
              className="w-full cursor-pointer file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-gray-100 file:text-sm"
              required />
            <p className="text-xs text-gray-500 mt-1">
              Must have a matching <code>.tbi</code> (same basename) in the same folder.
            </p>
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">Ambiguities</label>
            <select value={ambiguities} onChange={(e) => setAmbiguities(e.target.value)}
        ...
        <button type="submit" disabled={loading || !bedFile || !gtfBgz}
      ...
      {summary && (
        <section className="mt-8 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm"><span className="font-medium">Unique genes: </span>{summary.unique_genes}</div>
          </div>

          {/* Two plots: Biotype + Chromosome */}
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="border rounded-2xl p-4 bg-white h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={biotypeChartData} margin={{ top: 20, right: 30, left: 30, bottom: 100 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="biotype" angle={-40} textAnchor="end" height={100}>
                    <Label value="Biotype" offset={60} position="bottom"  />
                  </XAxis>
                  <YAxis>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={(() => {
                    ...
                    return order.filter((c) => counts[c] > 0).map((c) => ({ chrom: c, count: counts[c] }));
                  })()}
                  margin={{ top: 10, right: 20, left: 20, bottom: 60 }}
                >
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="chrom" angle={-30} textAnchor="end" height={70}>
          ...
          {geneNames.length > 0 && (
            <div className="border rounded-2xl p-4 bg-white">
              ...
              <div className="text-sm text-gray-700 whitespace-nowrap overflow-x-auto">{geneNames.join(", ")}</div>
            </div>
          )}

          {csvPath && (
            <a href={`/api/download?path=${encodeURIComponent(csvPath)}`}
          ...
          <div className="border rounded-2xl bg-white max-h-[520px] overflow-y-auto">
            <table className="min-w-full text-sm text-gray-800">
              <thead className="bg-gray-100 sticky top-0">
                <tr>
                  {["chr","start","end","hugo/gene","strand","biotype",
                    "tx_overlap%","exon_overlap%","cds_overlap%","transcript_id"].map((h) => (
